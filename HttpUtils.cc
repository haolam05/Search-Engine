...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  char test_abs_path[PATH_MAX];
  if (realpath(root_dir.c_str(), root_abs_path) == NULL ||
      realpath(test_file.c_str(), test_abs_path) == NULL ||
      strlen(root_abs_path) > strlen(test_abs_path) ||
      strncmp(root_abs_path, test_abs_path, strlen(root_abs_path)) != 0 ||
      test_abs_path[strlen(root_abs_path)] != '/') {
    return false;
  }
